...
function buildAllModelInstructions(rootDir, modelsConfig, sources,
  ...
  var instructions = modelNamesToBuild
    .map(function createModelInstructions(name) {
      ...
      if (!config && definition && definition.definition && definition.definition.customizedModel) {
          config = modelsConfig[definition.definition.customizedModel];
      }
...
function verifyModelDefinitions(rootDir, modelDefinitions) {
  ...
  modelDefinitions.forEach(function(definition, idx) {
    ...
    if (entry.definition.customModel && registry[modelName]) {
        var save = registry[modelName];
        save.definition.name = 'Base' + modelName;
        if (!entry.definition.plural) {
            entry.definition.plural = save.definition.plural;
        }
        save.definition.plural = save.definition.name + save.definition.plural;
        save.definition.customizedModel = modelName;
        entry.definition.base = save.definition.name;
        registry[save.definition.name] = save;
    }
